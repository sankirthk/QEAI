...
export const LaserFlow: React.FC<Props> = ({
  ...
  const currentRGB = useRef(new THREE.Vector3(1, 1, 1)); // start with white
  const targetRGB = useRef(new THREE.Vector3(1, 1, 1));

  useEffect(() => {
    const colorList = ["#CF9EFF", "#6EDCFF", "#FF9A8B", "#A8FF60"];
    const updateColor = () => {
      currentColorIndex.current =
        (currentColorIndex.current + 1) % colorList.length;
      const hex = colorList[currentColorIndex.current];
      const colorVec = new THREE.Color(hex);
      targetRGB.current.set(colorVec.r, colorVec.g, colorVec.b);
    };

    updateColor(); // ✅ Ensure the first color is set immediately
    const colorInterval = setInterval(updateColor, 10000);
    return () => clearInterval(colorInterval); // ✅ Clean up
  }, []);
  ...
  useEffect(() => {
    ...
    const animate = () => {
      ...
      currentRGB.current.lerp(targetRGB.current, 0.05); // try 0.05–0.1 for speed/smoothness
      uniforms.uColor.value.copy(currentRGB.current);
      ...
      uniforms.uFogTime.value = fogTimeRef.current;
      ...
      const colorDelta = targetRGB.current
        .clone()
        .sub(currentRGB.current)
        .length();
      if (colorDelta > 0.05) {
        uniforms.uWIntensity.value = wispIntensity * 0.6;
        uniforms.uFogIntensity.value = fogIntensity * 0.6;
      } else {
        uniforms.uWIntensity.value = wispIntensity;
        uniforms.uFogIntensity.value = fogIntensity;
      }
